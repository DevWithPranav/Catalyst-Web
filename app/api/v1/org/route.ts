...
export async function GET(request: Request) {
  try {
    ...
    const orgs = await database.listDocuments(DB_ID, COLLECTIONS.ORGANIZATIONS, queries);

    return NextResponse.json({
      documents: orgs.documents,
      total: orgs.total,
      page: Math.floor(pagination.offset / pagination.limit) + 1,
      limit: pagination.limit,
    });
  } catch (error) {
    return handleError("Fetch organizations", error);
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    return handleError("Create organization", error);
  }
}
